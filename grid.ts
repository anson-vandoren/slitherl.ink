...
export class Grid {
  ...
  isFirmlyForced(
    ...
    const key = this.getCanonicalEdgeKey(q, r, dir);
    if (visited.has(key)) return false; // Cycle detected, not firm

    const state = this.getEdgeState(q, r, dir);
    if (state === EdgeState.ACTIVE || state === EdgeState.OFF) return true; // User/Permanent state is firm
    if (state === EdgeState.UNKNOWN) return false;
  ...
  checkEdgeFirmness(q: number, r: number, dir: EdgeDirection, visited: Set<string>): boolean {
    const hex = this.getHex(q, r);
    if (!hex) return true; // Should not happen for valid map edges

    const [v1, v2] = verticesForEdge(dir);
    if (this.checkVertexFirmness(hex, v1, dir, visited)) return true;
    if (this.checkVertexFirmness(hex, v2, dir, visited)) return true;
  ...
  checkVertexFirmness(
    hex: Hex,
    vIndex: VertexDirection,
    targetDir: EdgeDirection,
    visited: Set<string>
  ): boolean {
    const ctx = this.getVertexState(hex, vIndex);

    let other1State: EdgeState;
    let other1FirmFn: () => boolean;
    let other2State: EdgeState;
    let other2FirmFn: () => boolean;
    ...
    if (ctx.e1Index === targetDir) {
      ...
      other2State = ctx.s3;
      other2FirmFn = () => {
        if (!ctx.neighborContext) return true; // Boundary is firm
        return this.isFirmlyForced(
    ...
    if (this.forcesOff(other1State, other2State)) {
      ...
      if (other1FirmFn() && other2FirmFn()) return true;
    }
  ...
  checkVertex(hex: Hex, cornerIndex: VertexDirection) {
    ...
    const isFirm = (
      ...
      if (state === EdgeState.ACTIVE || state === EdgeState.OFF) return true;
      if (state === EdgeState.UNKNOWN) return false;
      // CALCULATED_OFF
      if (neighborCtx === undefined && dir === undefined) return true; // Boundary implicit
    ...
    const inputsAreFirm = (targetDir: EdgeDirection) => {
      if (targetDir === ctx.e1Index) {
        // Inputs s2, s3
        if (!isFirm(ctx.s2, hex.q, hex.r, ctx.e2Index)) return false;
        if (!ctx.neighborContext) return true; // s3 is Boundary (Firm)
        return isFirm(
          ctx.s3,
          ctx.neighborContext.hex.q,
          ctx.neighborContext.hex.r,
        ...
        // Inputs s1, s3
        if (!isFirm(ctx.s1, hex.q, hex.r, ctx.e1Index)) return false;
        if (!ctx.neighborContext) return true; // s3 is Boundary (Firm)
        return isFirm(
          ctx.s3,
          ctx.neighborContext.hex.q,
