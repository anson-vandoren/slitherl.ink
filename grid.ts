...
export class Grid {
  ...
  isGrounded(hex: Hex, edgeIndex: EdgeDirection, visited: Set<string> = new Set()): boolean {
    ...
    if (visited.has(edgeId)) return false; // Cycle detected
    visited.add(edgeId);
    ...
    const checkVertexForce = (corner: VertexDirection): boolean => {
      ...
      // Neighbor stuff
      const dirN1toN2 = ((corner + 1) % 6) as EdgeDirection;
      const dirN2toN1 = ((corner + 4) % 6) as EdgeDirection;
  ...
  getVertexState(hex: Hex, cornerIndex: VertexDirection) {
    ...
    return {
      ...
      forcesE1: forcesOff(
        ...
        e2Index,
        effectiveS3,
        neighborContext?.hex ?? null,
        neighborContext?.edgeIndex ?? null
      ),
      s2,
      e2Index,
      forcesE2: forcesOff(
  ...
  checkVertex(hex: Hex, cornerIndex: VertexDirection) {
    ...
    const otherEndForcesOff = (h: Hex, edgeIdx: EdgeDirection, currentCorner: number): boolean => {
      ...
      if (otherCtx.e1Index === edgeIdx) return otherCtx.forcesE1;
      if (otherCtx.e2Index === edgeIdx) return otherCtx.forcesE2;
      return false;
    };
    ...
    if (ctx.neighborContext) {
      if (ctx.s3 === EdgeState.UNKNOWN && ctx.forcesS3) {
        ...
        const v1Forces =
          (startForcing.e1Index === ctx.neighborContext.edgeIndex && startForcing.forcesE1) ||
          (startForcing.e2Index === ctx.neighborContext.edgeIndex && startForcing.forcesE2);
        const v2Forces =
          (endForcing.e1Index === ctx.neighborContext.edgeIndex && endForcing.forcesE1) ||
  ...
  loadMap(mapData: { radius: number; hexes: Hex[] }) {
    this.hexagons.clear();
