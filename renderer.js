import { EdgeState, HexColor } from './grid.js';
const COLORS = {
    bg0_hard: '#1d2021',
    bg0: '#282828',
    bg0_soft: '#32302f',
...
export class Renderer {
    ...
    drawHexEdges(ctx, hex, corners, layerType) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (layerType === 'inactive' &&
                (state === EdgeState.OFF || state === EdgeState.CALCULATED_OFF))
                matchesLayer = true;
            else if (layerType === 'neutral' && state === EdgeState.UNKNOWN)
                matchesLayer = true;
            else if (layerType === 'active' && state === EdgeState.ACTIVE)
                matchesLayer = true;
            if (!matchesLayer)
                continue;
            const p1 = corners[i];
